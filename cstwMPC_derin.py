# -*- coding: utf-8 -*-
"""
Created on Sat Mar 11 19:08:59 2017
...
from Impatience_derin import ImpatienceCondition
...
from ConsIndShockModel import solveConsKinkedR
import SetupParamsCSTW_derin as Params
...
class cstwMPCagent(Model.IndShockConsumerType):
    ...
    def __init__(self,time_flow=True,**kwds):
        ...
        **kwds : keyword arguments
        ...
        self.time_inv_.remove('Rfree')
        self.solveOnePeriod = solveConsKinkedR
        self.update()
    ...
    def simulateCSTW(self):
        ...
        self.initializeSim()
        self.simConsHistory()
        self.W_history = self.pHist*self.bHist/self.Rsave
...
for nn in range(num_consumer_types):
    ...
    SpreadRconsumerTypes.append(newType)    
...
assignRdistribution(SpreadRconsumerTypes,Rsave_list,False)               # assigns Rsave
...
def makeCSTWstats(DiscFac,nabla,this_type_list,age_weight,lorenz_distance=0.0,save_name=None):
    ...
    sim_weight_all = np.tile(np.repeat(age_weight,this_type_list[0].Nagents),Params.total_types)
    
    if Params.do_beta_dist and Params.do_lifecycle:
        kappa_mean_by_age_type = (np.mean(np.vstack((this_type.kappa_history for this_type in this_type_list)),axis=1)).reshape((Params.total_types*3,DropoutType.T_total+1))
        kappa_mean_by_age_pref = np.zeros((Params.total_types,DropoutType.T_total+1)) + np.nan
        for j in range(Params.total_types):
        ...
        kappa_hi_beta_by_age = kappa_mean_by_age_pref[Params.total_types-1,:]
    ...
    kappa_all = calcWeightedAvg(np.vstack((this_type.kappa_history for this_type in this_type_list)),np.tile(age_weight/float(Params.total_types),Params.total_types))
...
def calcKappaMean(DiscFac,nabla):
    ...
    kappa_all = calcWeightedAvg(np.vstack((this_type.kappa_history for this_type in est_type_list)),
                                np.tile(Params.age_weight_all/float(Params.total_types),
                                        Params.total_types))
    return kappa_all
...
if __name__ == "__main__":
    ...
    # Expand the estimation type list if doing Rsave-dist
    if Params.do_Rsave_dist:
        SpreadR_type_list = []
        for nn in range(Params.num_consumer_types):
            SpreadR_type_list += deepcopy(short_type_list)
        Rsave_list = [1.015, 1.019, 1.022]
#       Rsave_list  = approxUniform(N=Params.num_consumer_types,bot=Params.bottomRsave,top=Params.topRsave)[1]   # only take values, not probs
#       Rsave_list.sort() # sort the list from smaller to larger saving rate
        assignRdistribution(SpreadR_type_list,Rsave_list)               # assigns Rsave
        est1_type_list = SpreadR_type_list
    else:
        est1_type_list = short_type_list
    
    # Expand the estimation type list if doing beta-dist
    if Params.do_beta_dist:
        long_type_list = []
        for j in range(Params.pref_type_count):
            long_type_list += deepcopy(est1_type_list)
        est_type_list = long_type_list
    else:
        est_type_list = est1_type_list
    ...
    def betaDistObjective(nabla):
        ...
        if Params.do_tractable:
            ...
            top = (InfiniteType.PermGroFac[0]/(Rsave_list[2]*1.003266*0.99375))
        DiscFac_new = brentq(intermediateObjective,0.90,top,xtol=10**(-4))
        N=Params.total_types
    ...
    if Params.run_estimation:
        ...
        ImpatienceCondition(CRRA=Params.CRRA,DiscFac=DiscFac,N=Params.pref_type_count,nabla=nabla,Rsave_list=Rsave_list,PermShkStd=InfiniteType.PermShkStd,PermGroFac=InfiniteType.PermGroFac[0],Unemp=InfiniteType.UnempPrb,PermShkCount=InfiniteType.PermShkCount,PermShkDstn=InfiniteType.PermShkDstn)
