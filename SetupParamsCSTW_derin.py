...
do_Rsave_dist = True
num_consumer_types   = 3       # declare the number of types we want
bottomRsave = 1.005
topRsave    = 1.043
...
if do_beta_dist:
    pref_type_count = 2       # Number of discrete beta types in beta-dist
else:
    pref_type_count = 1       # Just one beta type in beta-point
total_types = num_consumer_types * pref_type_count
...
IncUnemp = 0.0               # Unemployment benefit replacement rate
...
init_infinite = {"CRRA":CRRA,
                ...
                "Rboro": 1.10/LivPrb_i[0],           # Interest factor when borrowing
                "Rsave": 1.03/LivPrb_i[0],           # Interest factor when saving
...
# -----------------------------------------------------------------------------
r                                   # Apply to [Rboro,Rsave] the same transformation 
                                   # used for Rfree in init_infinite
Rboro = 1.15/LivPrb_i[0]           # Interest factor when borrowing
Rsave = 1.03/LivPrb_i[0]           # Interest factor when saving
bottomRboro = 1.1/LivPrb_i[0]    # Used in the function 'RboroDistObjective' when calling brentq
